...
final class JestUnitTestEngine extends ArcanistUnitTestEngine {
  const PROCESSOR = '/node_modules/jest-phabricator/build/index.js';
  const JEST_PATH = '/support/phabricator/src/extensions/jest.sh';
  ...
  private function runJSTests() {
    ...
    if (count($jest_paths) > self::GIGANTIC_DIFF_THRESHOLD) {
      ...
      if (count($jest_paths) > 0) {
        $console->writeOut("Running Jest tests.\n");
        $commands[] = array(
          'bin' => $this->getRoot() . self::JEST_PATH,
      ...
      try {
        ...
        $result->setName('mordor-batch tests');
  ...
  private function getJestOptions($paths) {
    ...
    $options = array(
      ...
      '--testResultsProcessor=' . $this->getRoot() . self::PROCESSOR
    );
